...
if(isset($_GET['delete'])){
    $id = $_GET['delete'];
    $sql = "DELETE FROM subject_comb WHERE comb_id = $id";
    $result = mysqli_query($conn, $sql);
    if($result){
...
<?php if($row['status']=='0')
{ ?>
<a href="manage-subjcomb.php?acid=<?php echo $row['scid'];?>" onclick="return confirm('Do you really want to activate this subject?');"><i class="fa fa-check" title="Activate Record" style="color: #28a745; margin-right: 10px;"></i> </a><?php } else {?>

<a href="manage-subjcomb.php?did=<?php echo $row['scid'];?>" onclick="return confirm('Do you really want to deactivate this subject?');"><i class="fa fa-times" title="Deactivate Record" style="color: #ffc107; margin-right: 10px;"></i> </a>
<?php }?>
<a href="manage-subjcomb.php?delete=<?php echo $row['scid'];?>" onclick="return confirm('Do you really want to delete this combination?');"><i class="fa fa-trash" title="Delete Record" style="color: #dc3545;"></i> </a>
</td>        
</tr>
        <?php
        $c++;
